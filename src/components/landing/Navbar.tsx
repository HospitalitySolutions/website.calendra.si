...
const Navbar = () => {
  ...
  const industriesActive = industryPaths.includes(pathname);
      ...
      <div className="container mx-auto flex h-20 items-center justify-between gap-3 px-4 lg:px-8">
        ...
        <div className="hidden items-center gap-5 xl:flex">
          ...
          <div className="group relative">
            <a href={`${homePath}#za-koga`} className={`relative inline-flex items-center gap-1 whitespace-nowrap py-7 text-sm font-medium transition-colors hover:text-foreground ${industriesActive ? "text-foreground" : "text-muted-foreground"}`}>
              {solutionsLabel}<ChevronDown className="h-4 w-4 transition group-hover:rotate-180 group-focus-within:rotate-180" aria-hidden="true" />
              {industriesActive && <span className="absolute bottom-0 left-0 h-0.5 w-full rounded-full bg-primary" aria-hidden="true" />}
            </a>
            <div className="invisible absolute left-1/2 top-[calc(100%-4px)] z-50 w-[360px] -translate-x-1/2 translate-y-2 rounded-2xl border border-border/70 bg-background p-3 opacity-0 shadow-xl transition group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">
              <a href={`${homePath}#za-koga`} className="block rounded-xl bg-primary/[0.06] px-4 py-3 font-semibold text-foreground transition hover:bg-primary/[0.1]">
        ...
        <div className="border-t border-border bg-card px-4 pb-6 pt-4 xl:hidden">
          <div className="flex flex-col gap-3">
            ...
            <div className="rounded-2xl border border-border/60 bg-background p-2">
              <a href={`${homePath}#za-koga`} className={`block rounded-xl px-3 py-2.5 font-semibold ${industriesActive ? "bg-primary/[0.08] text-primary" : "text-foreground"}`} onClick={() => setOpen(false)}>{solutionsLabel}</a>
              <div className="mt-1 grid gap-1 border-l border-border pl-3">
                {INDUSTRY_ROUTE_KEYS.map((routeKey) => <a key={routeKey} href={getRoutePath(routeKey, language)} className="rounded-lg px-3 py-2 text-sm leading-5 text-muted-foreground hover:bg-secondary hover:text-foreground" onClick={() => setOpen(false)}>{getIndustryContent(routeKey, language).navLabel}</a>)}
              </div>
            </div>
